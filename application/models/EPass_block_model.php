...
class EPass_block_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $epass_block_table.*, events.title as event_name, epass_area.area_name as area_name, epass_area.event_id as event_id, (SELECT COUNT(id) 
        FROM epass_seat WHERE epass_seat.block_id = $epass_block_table.id AND epass_seat.deleted = '0') as `seats`
        FROM $epass_block_table 
            LEFT JOIN epass_area ON epass_area.id = $epass_block_table.area_id
