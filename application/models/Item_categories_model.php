...
class Item_categories_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $item_categories_table.*, TRIM(CONCAT(users.first_name, ' ', users.last_name)) AS full_name
        FROM $item_categories_table
        LEFT JOIN users ON users.id = $item_categories_table.created_by
