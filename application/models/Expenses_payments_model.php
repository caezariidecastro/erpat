...
class Expenses_payments_model extends Crud_model {
    ...
    function get_total_payments($expense_id){
        ...
        $sql = "SELECT SUM(amount) AS total 
            FROM $expenses_payments_table
            WHERE $expenses_payments_table.deleted=0 
                AND $expenses_payments_table.expense_id = $expense_id";

        $has_payments = $this->db->query($sql);
        return $has_payments ? $has_payments->row()->total : 0.00;
    }
}
