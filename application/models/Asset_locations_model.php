...
class Asset_locations_model extends Crud_model {
    ...
    function get_parenting_count($id){
        $asset_locations_table = $this->db->dbprefix('asset_locations');
        $sql = "SELECT COUNT(parent1.id) + COUNT(parent2.id) + COUNT(parent3.id) + COUNT(parent4.id) AS parenting_count
        FROM $asset_locations_table
        ...
        LEFT JOIN $asset_locations_table parent3 ON parent3.id = parent2.parent_id
        LEFT JOIN $asset_locations_table parent4 ON parent4.id = parent3.parent_id
