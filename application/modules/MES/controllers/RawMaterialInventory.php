...
class RawMaterialInventory extends MY_Controller {
    ...
    function index(){
        $this->template->rander("raw_materials/inventory/index");
    }
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                <div class="text-off pull-right text-right">
                                    Stocks on hand: '.(get_current_material_inventory_count($data)).'
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    function add_material_inventory_modal_form() {
        ...
        $this->load->view('raw_materials/inventory/add_material_inventory', $view_data);
    }
    ...
    function add_stock_modal_form($warehouse_id, $material_inventory_id) {
        ...
        $this->load->view('raw_materials/inventory/add_stock', $view_data);
    }
    ...
    private function _material_inventory_make_row($data) {
        $detail = '<div class="pull-left material-row" data-id="'.$data->id.'">
                    <div class="media-body">
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="text-off pull-right text-right">
                                    Available stocks: '.(get_current_material_inventory_count($data)).'
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("mes/RawMaterialInventory/delete"), "data-action" => "delete-confirmation", "data-reload-on-success" => "1")) . '</li>';
        $add = '<li role="presentation">' . modal_anchor(get_uri("mes/RawMaterialInventory/add_stock_modal_form/$data->warehouse/$data->id"), "<i class='fa fa-plus-circle'></i> " . lang('add_stock'), array( "title" => lang('add_stock'), "id" => "add_stock_button")) . '</li>';
    ...
    function material_details($id){
        $data = $this->load->view('raw_materials/inventory/material_details', array('id'=> $id));
