...
<div class="invoice-preview-container bg-white mt15">
    ...
    <table id="production-materials-table" class="table-responsive" style="width: 100%; color: #444;">            
        <tr style="font-weight: bold; background-color: <?php echo $color; ?>; color: #fff;  ">
            <th style="width: 45%; border: 1px solid #eee; padding: 12px;"> <?php echo lang("material"); ?> </th>
            <th style="text-align: center;  width: 15%; border: 1px solid #eee; padding: 12px; padding: 12px;"> <?php echo lang("unit_type"); ?></th>
            <th style="text-align: center;  width: 20%; border: 1px solid #eee; padding: 12px;"> <?php echo lang("quantity"); ?></th>
            <th style="text-align: center;  width: 20%; border: 1px solid #eee; padding: 12px;"> <?php echo lang("buffer"); ?></th>
            <th style="text-align: center; border: 1px solid #eee; width: 20%; padding: 12px;"> <?php echo lang("total"); ?></th>
        </tr>
        <?php
        foreach ($bill_of_material_materials as $material) {
            $buffer = $production_info->buffer ? $production_info->quantity * round($production_info->buffer / 100, 2) : "";
            $quantity = $production_info->bill_of_material_quantity . " x " . $production_info->quantity;
            $total = $production_info->bill_of_material_quantity * $production_info->quantity + $buffer;
