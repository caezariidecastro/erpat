<?php echo form_open(get_uri("purchase_orders/send_purchase"), array("id" => "send-purchase-form", "class" => "general-form", "role" => "form")); ?>
<div id="send_purchase-dropzone" class="post-dropzone">
    <div class="modal-body clearfix">
        ...
        <div class="form-group ml15">
            <i class='fa fa-check-circle' style="color: #5CB85C;"></i> <?php echo lang('attached') . ' ' . anchor(get_uri("purchase_orders/download_pdf/" . $purchase_info->id), lang("purchase") . "-$purchase_info->id.pdf", array("target" => "_blank")); ?> 
        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        var uploadUrl = "<?php echo get_uri("purchase_orders/upload_file"); ?>";
        var validationUri = "<?php echo get_uri("purchase_orders/validate_purchases_file"); ?>";
        ...
        $("#contact_id").select2().on("change", function () {
            ...
            if (contact_id) {
                ...
                appLoader.show();
                $.ajax({
                    url: "<?php echo get_uri('purchase_orders/get_send_purchase_template/' . $purchase_info->id) ?>" + "/" + contact_id + "/json",
                    dataType: "json",
