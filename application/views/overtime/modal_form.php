<?php echo form_open(get_uri("hrs/overtime/save"), array("id" => "overtime-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <input type="hidden" name="id" value="<?php echo $model_info->id; ?>" />
    <input type="hidden" name="note_override" value="Manual Override" />
    ...
    <div class="clearfix">
        <div class="form-group">
            ...
            <div class=" col-md-9">
                ...
                if (isset($team_members_info)) {
                    ...
                    echo form_dropdown("user_id", $team_members_dropdown, "", "class='select2 validate-hidden' id='overtime_user_id' data-rule-required='true', data-msg-required='" . lang('field_required') . "'");
                }
                ?>
            </div>
        </div>
        ...
        <div class="col-md-4 col-sm-4 form-group">
            <?php
            $start_time = (is_date_exists($model_info->start_time)) ? convert_date_utc_to_local($model_info->start_time) : "";

            if ($time_format_24_hours) {
                $in_time_value = $start_time ? date("H:i", strtotime($start_time)) : "";
            } else {
                $in_time_value = $start_time ? convert_time_to_12hours_format(date("H:i:s", strtotime($start_time))) : "";
            }

            echo form_input(array(
                "id" => "in_date",
                "name" => "in_date",
                "value" => $start_time ? date("Y-m-d", strtotime($start_time)) : "",
                "class" => "form-control",
    ...
    <div class="clearfix">
        ...
        <div class=" col-md-4 col-sm-4 form-group">
            <?php
            $end_time = is_date_exists($model_info->end_time) ? convert_date_utc_to_local($model_info->end_time) : "";

            if ($time_format_24_hours) {
                $out_time_value = $end_time ? date("H:i", strtotime($end_time)) : "";
            } else {
                $out_time_value = $end_time ? convert_time_to_12hours_format(date("H:i:s", strtotime($end_time))) : "";
            }

            echo form_input(array(
                "id" => "out_date",
                "name" => "out_date",
                "value" => $end_time ? date("Y-m-d", strtotime($end_time)) : "",
                "class" => "form-control",
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        if ($("#overtime_user_id").length) {
            $("#overtime_user_id").select2();
        }
