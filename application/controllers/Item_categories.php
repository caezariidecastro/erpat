...
class Item_categories extends MY_Controller {
    ...
    function index(){
        ...
        $this->template->rander("item_categories/index", $view_data);
    }
    ...
    private function _make_row($data) {
        return array(
            ...
            modal_anchor(get_uri("item_categories/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_category'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contribution'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("item_categories/delete"), "data-action" => "delete-confirmation"))
        );
    }
    ...
    function save() {
        ...
        $contribution_id = $this->Item_categories_model->save($contribution_data, $id);
        if ($contribution_id) {
            $options = array("id" => $contribution_id);
            $contribution_info = $this->Item_categories_model->get_details($options)->row();
