...
class Attendance extends Users {
    ...
    function logtime($userid) {
        ...
        $is_currently_clocked_in = $this->Attendance_model->log_time($user_id, "System Generated", true);  
    ...
    function my_clocked_in_list() {        
        ...
        $list_response = [];
        foreach($list_data as $item) {
            if(isset($item->out_time) && !empty(isset($item->out_time))) {
                $list_item = new stdClass();
                $list_item->duration = convert_seconds_to_time_format(abs(strtotime($item->out_time) - strtotime($item->in_time)), true);
                $list_item->in_time = convert_date_utc_to_local($item->in_time, "Y-m-d g:i A");
                $list_item->out_time = convert_date_utc_to_local($item->out_time, "Y-m-d g:i A");
