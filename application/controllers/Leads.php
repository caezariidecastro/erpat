...
class Leads extends MY_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array(
            anchor(get_uri("mcs/leads/view/" . $data->id), $data->company_name),
        ...
        $row_data[] = modal_anchor(get_uri("mcs/leads/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_lead'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_lead'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("mcs/leads/delete"), "data-action" => "delete-confirmation"));
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("mcs/leads/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("mcs/leads/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("mcs/leads/delete_file"), "data-action" => "delete-confirmation"));
    ...
    private function _make_contact_row($data, $custom_fields) {
        ...
        $contact_link = anchor(get_uri("mcs/leads/contact_profile/" . $data->id), $full_name . $primary_contact);
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("mcs/leads/delete_contact"), "data-action" => "delete"));
    ...
    function save_as_client() {
        ...
        if ($client_id) {
            ...
            if ($save_client_id) {
                ...
                echo json_encode(array("success" => true, 'redirect_to' => get_uri("sales/Clients/view/$save_client_id"), "message" => lang('record_saved')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        }
