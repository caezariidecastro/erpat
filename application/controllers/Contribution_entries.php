...
class Contribution_entries extends MY_Controller {
    ...
    private function _make_row($data) {
        ...
        $edit = '<li role="presentation">' . modal_anchor(get_uri("contribution_entries/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), array("class" => "edit", "title" => lang('edit'), "data-post-id" => $data->id)) . '</li>';
        $delete = "";
        $pay = "";
        $cancel = "";
        $pdf = "";
        ...
        if($data->status == "not paid"){
            $pay = '<li role="presentation">'. js_anchor("<i class='fa fa-check'></i> " . lang('mark_as_paid'), array('title' => lang('update'), "class" => "", "data-action-url" => get_uri("contribution_entries/pay/$data->id"), "data-action" => "update")) .'</li>';
            $delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i> " . lang('delete'), array('title' => lang('delete_entry'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("contribution_entries/delete"), "data-action" => "delete-confirmation")) . '</li>';
        }

        if($data->status == "paid"){
            $pdf = "<li role='presentation'>" . anchor(get_uri("contribution_entries/pdf/".$data->id), "<i class='fa fa-file-pdf-o'></i> " . lang('view_pdf'), array("title" => lang('view_pdf'), "target" => "_blank")) . "</li>";
            $cancel = '<li role="presentation">'. js_anchor("<i class='fa fa-remove'></i> " . lang('mark_as_cancelled'), array('title' => lang('update'), "data-action-url" => get_uri("contribution_entries/cancel/$data->id"), "data-action" => "update"));
        }
