...
class Contribution_entries extends MY_Controller {
    ...
    private function _make_row($data) {
        ...
        if($data->status == "not paid" || $data->status == "partially paid"){
            $pay = '<li role="presentation">'. modal_anchor(get_uri("fas/contribution_entries/payment_modal_form/$data->id"), "<i class='fa fa-check'></i> " . lang('add_payment'), array("class" => "edit", "title" => lang('add_payment'), "data-post-id" => $data->id)).'</li>';
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $contribution_info = $this->Contribution_entries_model->get_details($options)->row();
        return $this->_make_row($contribution_info);
    }
    ...
    function payment_modal_form($id = 0) {
        ...
        $this->load->view('contribution_entries/payment_modal_form', $view_data);
    }
    ...
    function pay($contribution_id = 0) {
        if ($contribution_id) {
            ...
            $expense_balance = $this->get_expense_balance($contribution_info->amount, $contribution_info->expense_id);
            ...
            $expense_payment_data = array(
                "expense_id" => $contribution_info->expense_id,
                "account_id" =>  $this->input->post('account_id'),
            ...
            if( $expense_payment_id && (float)$payment_amount >= (float)$expense_balance ) {
                $contribution_data["status"] = "paid"; //Fully paid
