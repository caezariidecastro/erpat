...
class ManufacturingOrders extends MY_Controller {
    ...
    function save() {
        ...
        if(!$id){
            $production_data["created_on"] = get_current_utc_time();
    ...
    function modal_form() {
        ...
        $this->load->view('manufacturing_orders/modal_form', $view_data);
    }
    ...
    function modal_details($id = 0) {
        if ($id) {
            ...
            $view_data['bill_of_material_materials'] = $this->Bill_of_materials_model->get_materials( array("id" => $view_data["production_info"]->bill_of_material_id) );

            $this->load->view("manufacturing_orders/modal_details", $view_data);
        } else {
    ...
    function print($id = 0) {
        if ($id) {
            $view_data["production_info"] = $this->Productions_model->get_details(array("id" => $id))->row();
            $view_data['bill_of_material_materials'] = $this->Bill_of_materials_model->get_materials(array("id" => $view_data["production_info"]->bill_of_material_id));
            $view_data["details"] = $this->load->view("manufacturing_orders/details", $view_data, true); 

            $this->template->rander("manufacturing_orders/print", $view_data);
        }
    }
}
