...
class Purchase_order_returns extends MY_Controller {
    ...
    private function _make_row($data) {
        return array(
            anchor(get_uri("pid/returns/view/". $data->id), get_purchase_return_id($data->id)),
            anchor(get_uri("pid/purchases/view/". $data->purchase_id), get_purchase_order_id($data->purchase_id)),
            get_supplier_contact_link($data->vendor_id, $data->vendor_name),
            number_with_decimal($data->amount),
            nl2br($data->remarks),
            $data->created_on,
            get_team_member_profile_link($data->created_by, $data->creator_name, array("target" => "_blank")),
            modal_anchor(get_uri("purchase_order_returns/modal_form"), "<i class='fa fa-pencil'></i> ", array( "title" => lang('edit_return'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("purchase_order_returns/delete"), "data-action" => "delete-confirmation"))
        );
    }
    ...
    function view($id = 0){
        if($id){
            ...
            if ($view_data) {
                $this->template->rander("purchase_order_returns/view", $view_data);
            } else {
                show_404();
            }
        }
    ...
    function cancel_form(){
        ...
        $this->load->view('purchase_order_returns/cancel_form', $view_data);
    }
    ...
    function draft_form(){
        $view_data["id"] = $this->input->post("id");
        $this->load->view('purchase_order_returns/draft_form', $view_data);
    }
    ...
    function complete_form(){
        $view_data["id"] = $this->input->post("id");
        $this->load->view('purchase_order_returns/complete_form', $view_data);
    }
    ...
    function send_return_modal_form($purchase_return_id = 0) {
        if ($purchase_return_id) {
            ...
            $this->load->view('purchase_order_returns/send_return_modal_form', $view_data);
        } else {
            show_404();
        }
    }
    ...
    function get_send_return_template($purchase_return_id = 0, $contact_id = 0, $return_type = "", $purchase_return_info = "", $contact_info = "") {
        ...
        $parser_data["RETURN_URL"] = get_uri("pid/returns/preview/" . $purchase_return_info->id);
    ...
    function preview($purchase_return_id = 0, $show_close_preview = false) {
        if ($purchase_return_id) {
            ...
            $this->template->rander("purchase_order_returns/return_preview", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function print_return($purchase_return_id = 0) {
        if ($purchase_return_id) {
            ...
            echo json_encode(array("success" => true, "print_view" => $this->load->view("purchase_order_returns/print_return", $view_data, true)));
        } else {
